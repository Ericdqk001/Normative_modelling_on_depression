...
def process():
    ...
    parser.add_argument(
        "--epochs", type=int, default=1000, help="Number of epochs to train model"
    )
    parser.add_argument("--zdim", type=int, default=10, help="Number of latent vectors")
    parser.add_argument(
        "--hiddendim", type=list, default=[40], help="List of hidden layer sizes"
    )
    parser.add_argument("--batchsize", type=int, default=1000, help="Batch size")
    parser.add_argument(
        "--GPU", type=bool, default=True, help="Whether to use GPU for model training"
    )
    ...
    generator_train = torch.utils.data.DataLoader(
        train_dataset, batch_size=args.batchsize, shuffle=False
    )
    ...
    output_data["reconstruction_deviation"] = reconstruction_deviation(
        test_data.to_numpy(), test_prediction
    )
    ...
    for i in range(1, args.zdim + 1):
        ...
        for position, column in enumerate(rows):
            ax.boxplot(
                output_data[output_data["DX"] == column][
                    "latent_deviation_{0}".format(i - 1)
                ],
                positions=[position],
            )
    ...
    for position, column in enumerate(rows):
        ax.boxplot(
            output_data[output_data["DX"] == column]["reconstruction_deviation"],
            positions=[position],
        )
    ax.set_xticks(range(position + 1))
    ax.set_xticklabels(rows)
    ...
    for position, column in enumerate(rows):
        ax.boxplot(
            output_data[output_data["DX"] == column]["latent_deviation"],
            positions=[position],
        )
    ax.set_xticks(range(position + 1))
    ax.set_xticklabels(rows)
    ...
    age_pval.to_csv(
        join(args.outpath, "p_values_latentdeviations_vs_age.csv"), index=False
    )

    ICV_pval = latent_pvalues(test_latent, test_covariates["ICV"], type="continuous")
    ICV_pval.to_csv(
        join(args.outpath, "p_values_latentdeviations_vs_ICV.csv"), index=False
    )

    DX_pval = latent_pvalues(test_latent, test_covariates["DX"], type="discrete")
    DX_pval.to_csv(
        join(args.outpath, "p_values_latentdeviations_vs_DX.csv"), index=False
    )
